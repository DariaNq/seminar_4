...
def stop(update, context):
    ...
    context.bot.send_message(update.effective_chat.id, f"Игра окончена, чтобы начать заново напишите /start"
                                                       f"\n Узнать количество побед: /victory")

def victory(update, context):
    context.bot.send_message(update.effective_chat.id, f'Кол-во побед у бота: {count_bot_victory}\n'
                                                      f'Кол-во побед у {update.effective_user.first_name}: {count_user_victory}')


start_handler = CommandHandler('start', start)
still_handler = CommandHandler('yet', yet)
stop_handler = CommandHandler('stop', stop)
victory_handler = CommandHandler('victory', victory)


dispatcher.add_handler(start_handler)
dispatcher.add_handler(still_handler)
dispatcher.add_handler(stop_handler)
dispatcher.add_handler(victory_handler)
